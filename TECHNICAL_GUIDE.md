...
2. Initialize Environment and Generate Mock Data:
    The `setup.sh` script creates a virtual environment and installs all packages from `requirements.txt`. `generate_mock_data.py` creates a synthetic dataset (`mock_methylation.csv`) containing all simulated artifacts.
    ```bash
    bash setup.sh
    ...
    **Dependency management:** `requirements.txt` is autogenerated from `pyproject.toml` using `pip-compile` (pip-tools) and should not be edited by hand. To regenerate it after changing `pyproject.toml`:
    ```bash
    pip install pip-tools
    pip-compile pyproject.toml --output-file requirements.txt --no-annotate
    ```
