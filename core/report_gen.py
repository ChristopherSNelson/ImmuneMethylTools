...
2.  Artifact Simulation      qc_before_after.png (all 8 artifacts, before/after injection)
3.  Sample Exclusion         exclusion_accounting.png
4.  Global QC Metrics        qc_metrics.png
5.  Beta Distribution        beta_distribution_kde.png + normalization_before_after.png
6.  PCA                      pca_disease_label.png + pca_batch_id.png (stacked)
6b. PCA Covariate Panel      pca_covariates.png (batch / label / sex / age)
7.  Volcano Plots            volcano.png + volcano_clonal_risk.png (stacked)
8.  DETECTED Events Log      full bordered table from audit_log CSV
9.  Significant DMR Table    significant windows, amber rows for clonal_risk=True
"""
...
def generate_report(
    ...
    # ── Section 2: Artifact Simulation ──────────────────────────────────
    pdf.add_page()
    pdf.section("2. Artifact Simulation -- Before vs. After Injection")
    pdf.body(
        "Ten-panel figure confirming that each of the 8 stumper artifacts was "
        "correctly injected into the mock dataset. Each pair of panels shows the "
        "metric before (baseline) and after injection. Rows: (1) batch x disease "
        "confound + VDJ fragment scatter, (2) bisulfite failure rate + sample "
        "correlation heatmap, (3) contamination beta KDE, (4) low-coverage depth "
        "histogram + sex-metadata XCI signal, (5) FoxP3 x PAX5 lineage proxy scatter."
    )
    pdf.figure(
        os.path.join(FIGURES_DIR, "qc_before_after.png"),
        w=150,
        caption=(
            "Before/after injection fingerprints for all 8 artifacts. "
            "Generated by data/generate_mock_data.py; regenerate if stale."
        ),
    )

    # ── Section 3: Sample Exclusion Accounting ───────────────────────────
    pdf.add_page()
    pdf.section("3. Sample Exclusion Accounting")
    pdf.figure(
    ...
    # ── Section 4: Global QC Metrics ────────────────────────────────────
    pdf.add_page()
    pdf.section("4. Global QC Metrics")
    pdf.figure(
    ...
    # ── Section 5: Beta Distribution + Normalization ────────────────────
    pdf.add_page()
    pdf.section("5. Sample Purity -- Beta Distribution & Normalization")
    pdf.figure(
    ...
    pdf.figure(
        os.path.join(FIGURES_DIR, "normalization_before_after.png"),
        caption=(
            "Median-centring normalization: beta distributions before (left) "
            "and after (right) per-sample median subtraction. "
            "Post-normalization distributions should be centred near zero."
        ),
    )

    # ── Section 6: PCA ──────────────────────────────────────────────────
    pdf.add_page()
    pdf.section("6. Principal Component Analysis")
    pdf.figure(
    ...
    # ── Section 6b: PCA Covariate Panel ─────────────────────────────────
    pdf.add_page()
    pdf.section("6b. PCA -- Covariate Structure (Batch / Label / Sex / Age)")
    pdf.figure(
    ...
    pdf.add_page()
    pdf.section("7. Volcano Plots -- Differential Methylation Regions")
    pdf.figure(
    ...
    # ── Section 8: DETECTED Events Log ──────────────────────────────────
    pdf.add_page()
    pdf.section("8. DETECTED Events Log")
    pdf.body(
    ...
    # ── Section 9: Significant DMR Table ─────────────────────────────────
    pdf.add_page()
    pdf.section("9. Significant DMR Table")
