...
def generate_report(
    ...
    # Figure captions keyed by filename stem. Add an entry here whenever a
    # new qc_before_after_N.png is produced by generate_mock_data.py.
    _ARTIFACT_FIG_META = {
        "qc_before_after_1": (
    ...
    artifact_figs = sorted(glob.glob(os.path.join(FIGURES_DIR, "qc_before_after_*.png")))
    if not artifact_figs:
        pdf.body(
            "No qc_before_after_*.png figures found. "
            "Run data/generate_mock_data.py to generate them."
        )
    else:
        for idx, fig_path in enumerate(artifact_figs):
            stem = os.path.splitext(os.path.basename(fig_path))[0]
            intro, caption = _ARTIFACT_FIG_META.get(
                stem,
                (
                    f"Artifact simulation figure: {stem}.",
            ...
            if idx > 0:
                pdf.add_page()
            pdf.body(intro)
            pdf.figure(fig_path, w=165, caption=caption)
            pdf.ln(2)
